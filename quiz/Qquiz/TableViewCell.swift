...
class TableViewCell: UITableViewCell {
    ...
    @IBAction func button1Action(_ sender: UIButton) {
        ...
        let indexPath :NSIndexPath = (self.superview! as! UITableView).indexPath(for: self)! as NSIndexPath
           //for traking button1 indx for select state
           selectedStates.rows1.append(indexPath.row)
           selectedStates.buttonTags.append(indexPath.row)
            //selectedStates.temp[indexPath.row] = true
    ...
    @IBAction func button2Action(_ sender: UIButton) {
        ...
        let indexPath: NSIndexPath = (self.superview! as! UITableView).indexPath(for: self)! as NSIndexPath
        //for traking button1 indx for select state
        selectedStates.rows2.append(indexPath.row)
        selectedStates.buttonTags.append(indexPath.row)
        //selectedStates.temp[indexPath.row] = true
